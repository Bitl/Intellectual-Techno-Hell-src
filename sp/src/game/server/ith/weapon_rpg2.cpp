...
BEGIN_DATADESC( CWeaponRPG2 )

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CWeaponRPG2, DT_WeaponRPG2)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_rpg2, CWeaponRPG2 );
PRECACHE_WEAPON_REGISTER(weapon_rpg2);

acttable_t	CWeaponRPG2::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponRPG2);

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CWeaponRPG2::CWeaponRPG2()
{
	bFlip = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::Precache( void )
{
	BaseClass::Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::Activate( void )
{
	BaseClass::Activate();
}
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeaponRPG2::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
		case EVENT_WEAPON_SMG1:
		{
			...
			CMissile *entMissile = CMissile::Create( muzzlePoint, vecAngles, GetOwner()->edict() );	
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponRPG2::HasAnyAmmo( void )
{
...
//-----------------------------------------------------------------------------
bool CWeaponRPG2::WeaponShouldBeLowered( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::PrimaryAttack( void )
{
	if (!bFlip)
	{
		FireRightGun();
...
void CWeaponRPG2::SecondaryAttack(void)
{
	FireBothGuns();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::FireRightGun( void )
{
	// Can't be reloading
...
void CWeaponRPG2::FireLeftGun(void)
{
	// Can't be reloading
	if ( GetActivity() == ACT_VM_RELOAD )
		return;
...
void CWeaponRPG2::FireBothGuns(void)
{
	...
	if (pOwner->GetAmmoCount(m_iPrimaryAmmoType) > 1)
	{
		...
		CMissile *entMissile2 = CMissile::Create(muzzlePoint2, vecAngles, GetOwner()->edict());
...
// Input  : *pOwner - 
//-----------------------------------------------------------------------------
void CWeaponRPG2::DecrementAmmo( CBaseCombatCharacter *pOwner )
{
	// Take away our primary ammo type
	pOwner->RemoveAmmo( 1, m_iPrimaryAmmoType );
}

void CWeaponRPG2::Decrement2Ammo( CBaseCombatCharacter *pOwner )
{
	// Take away our primary ammo type
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::ItemPostFrame( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponRPG2::Holster( CBaseCombatWeapon *pSwitchingTo )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRPG2::Drop( const Vector &vecVelocity )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CWeaponRPG2::WeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponRPG2::WeaponRangeAttack1Condition( float flDot, float flDist )
{
