...
ConVar sv_regeneration_rate("sv_regeneration_rate", "99", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar sv_regeneration_limit("sv_regeneration_limit", "65", FCVAR_REPLICATED | FCVAR_CHEAT);
...
void CBasePlayer::DeathSound( const CTakeDamageInfo &info )
{
	...
	*/
	...
	//only emit our death scream.
	EmitSound("Player.DeathScream");
...
void CBasePlayer::PostThink()
{
	...
	if (IsAlive() && GetHealth() < GetMaxHealth() && (sv_regeneration.GetInt() == 1))
	{
		if (gpGlobals->curtime > m_fTimeLastHurt + sv_regeneration_wait_time.GetFloat())
		{
			...
			{
				if (sv_regeneration_limit.GetFloat() > 0 && GetHealth() >= sv_regeneration_limit.GetFloat())
				{
					m_fRegenRemander = 0;
				}
				else
...
void CBasePlayer::Precache( void )
{
	...
	PrecacheScriptSound("Player.DeathScream");
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
		...
		GiveAmmo(999, "AlyxGun");
		GiveAmmo(999, "CGuard");
		GiveAmmo(999, "GaussEnergy");
		GiveAmmo(999, "error_round");
		...
		//ith crap
		GiveNamedItem("weapon_errorlauncher");
		GiveNamedItem("weapon_errorlauncher2");
		GiveNamedItem("weapon_rpg2");
		GiveNamedItem("weapon_proto1");
		//ith2 shit
#ifdef ITH2
		GiveNamedItem("weapon_irifle");
#endif
		//make this the last weapon.
